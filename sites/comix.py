...
class ComixSiteHandler(BaseSiteHandler):
    ...
    def fetch_comic_context(self, url: str, scraper, make_request) -> SiteComicContext:
        # First, extract hash_id from URL (avoid fetching HTML up-front because the
        # title page is frequently protected and can return 403).
        hash_id = None
        ...
        # Try to fetch from API endpoint if we have hash_id (preferred path)
        if hash_id:
            try:
        ...
        # If API path didn't work, fall back to fetching and scraping HTML.
        if not manga_data:
        ...
        if not manga_data.get("cover") and soup is not None:
        ...
        if not manga_data.get("desc") and soup is not None:
    ...
    def get_chapter_images(self, chapter: Dict, scraper, make_request) -> List[str]:
        ...
        if not images:
            # Last-resort generic extraction of obvious image URLs from the HTML
            generic_matches = re.findall(r'"(https?://[^"\s]+\.(?:jpe?g|png|webp))"', html)
            if generic_matches:
                images = list(dict.fromkeys(generic_matches))

        if not images:
            # Final fallback: parse <img> tags directly from HTML
            try:
                soup = BeautifulSoup(html, "html.parser")
                candidates: List[str] = []
                ...
                # If BeautifulSoup fails for some reason, we'll still raise below
                pass
